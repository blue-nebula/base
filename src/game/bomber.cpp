...
namespace bomber
{
    ...
    void resetaffinity(int i, int value, const vec &pos)
    {
        ...
        if(f.enabled && !value)
        {
            ...
            if(isbomberaffinity(f))
            {
                ...
                game::announcef(S_V_BOMBRESET, CON_GAME, NULL, true, "\fathe \fs\fzwvbomb\fS has been reset");
            }
        }
    ...
    void scoreaffinity(gameent *d, int relay, int goal, int score)
    {
        ...
        game::announcef(S_V_BOMBSCORE, CON_GAME, d, true, "\fa%s destroyed the %s base for team %s%s (score: \fs\fc%d\fS, time taken: \fs\fc%s\fS)", game::colourname(d), gteam, game::colourteam(d->team), extra, score, timestr(lastmillis-f.inittime, 1));
    ...
    void takeaffinity(gameent *d, int i)
    {
        ...
        if(!f.droptime)
        {
            ...
            game::announcef(S_V_BOMBPICKUP, CON_GAME, d, true, "\fa%s picked up the \fs\fzwv\f($bombtex)bomb\fS", game::colourname(d));
        }
